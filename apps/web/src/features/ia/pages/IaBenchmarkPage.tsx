    ...
    <div className="relative hidden min-h-[250px] overflow-hidden lg:block">
      ...
      <div className="absolute right-8 top-8 w-[340px] rounded-xl border border-[hsl(var(--border))] bg-white p-5 shadow-[0_24px_80px_rgba(0,0,0,0.08)]">
        ...
        <div className="mt-5 space-y-4">
          {[
            { label: 'Vous', value: '78%', width: '78%', color: 'bg-[hsl(var(--primary))]' },
            { label: 'Médiane', value: '54%', width: '54%', color: 'bg-gray-300' },
            { label: 'Niveau 1', value: '38%', width: '38%', color: 'bg-gray-200' },
          ].map(({ label, value, width, color }) => (
            <div key={label}>
              <div className="mb-2 flex justify-between text-xs font-bold text-[hsl(var(--foreground-soft))]">
                <span>{label}</span>
                <span>{value}</span>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-[hsl(var(--accent))]">
      ...
      <div className="absolute bottom-8 right-20 h-px w-[440px] bg-gradient-to-r from-transparent via-[hsl(var(--primary)/0.3)] to-transparent" />
    </div>
  )
}
    ...
    <div className="space-y-6 px-4 py-5 sm:px-6 lg:px-8">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        {benchmarkSignals.map(({ title, text, icon: Icon, tone }) => (
          <article key={title} className="rounded-xl border border-[hsl(var(--border))] bg-background p-5">
