...
function PieTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 text-xs shadow-xl">
      ...
      <p className={d.pnl >= 0 ? 'text-green-500 tabular-nums' : 'text-red-500 tabular-nums'}>
...
function BarTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 text-xs shadow-xl">
      ...
      <p className={pnl >= 0 ? 'text-green-500 tabular-nums' : 'text-red-500 tabular-nums'}>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="rounded-lg border border-border bg-background p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-foreground">Répartition par instrument</h2>
            ...
            <div className="flex-1 space-y-1.5">
              {data!.bySymbol.slice(0, 6).map(item => (
                <div key={item.symbol} className="flex items-center gap-2">
                  ...
                  <span className="w-16 tabular-nums text-xs font-semibold text-muted-foreground">{item.symbol}</span>
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-accent">
                    <div className="h-full rounded-full" style={{ width: `${item.pct}%`, backgroundColor: item.color }} />
                  </div>
                  <span className="w-8 text-right text-xs text-muted-foreground">{item.pct.toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="rounded-lg border border-border bg-background p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-foreground">Performance par stratégie</h2>
          <span className="text-xs text-muted-foreground/60">{sourceLabel}</span>
        </div>
