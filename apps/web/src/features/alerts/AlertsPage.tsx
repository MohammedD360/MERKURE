...
function AlertCard({ alert }: { alert: Alert }) {
  ...
  return (
    ...
    >
      {!alert.isRead && (
        <span className="absolute right-4 top-4 h-2 w-2 rounded-full bg-[hsl(var(--primary))]" />
      )}
      ...
      <button
        ...
        className="absolute right-4 bottom-4 hidden rounded-lg p-1.5 text-muted-foreground/60 transition-colors hover:bg-[hsl(var(--accent))] hover:text-muted-foreground group-hover:flex"
      >
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="h-20 animate-pulse rounded-lg bg-[hsl(var(--accent))]" />
      ))}
    </div>
  )
}
    ...
    <div className="space-y-5 px-4 py-5 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
        <div className="flex items-center gap-3">
          ...
          {unreadCount > 0 && (
            <span className="rounded-full bg-[hsl(var(--primary))] px-2.5 py-0.5 text-xs font-bold text-white">
          ...
          <div className="inline-flex overflow-hidden rounded-md border border-border bg-white p-1">
            <button
              ...
              className={`rounded-lg px-4 py-2 text-xs font-semibold transition-colors ${
                !unreadOnly ? 'bg-[hsl(var(--primary))] text-white' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              Toutes
            </button>
