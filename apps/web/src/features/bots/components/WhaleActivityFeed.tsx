  ...
  return (
    <Card className="bg-card">
      <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
        ...
        <Badge className={`border-transparent text-xs ${live ? 'bg-[hsl(var(--chart-win)/0.18)] text-[hsl(var(--chart-win))]' : 'bg-white/10 text-white/50'}`}>
        ...
        {signals.map((s, i) => (
          <div key={`${s.wallet}-${i}`} className="flex items-center justify-between gap-3 py-2.5">
            ...
            <div className="flex shrink-0 items-center gap-3 text-right">
              ...
              <span className="w-16 text-xs text-white/40">{timeAgo(s.detectedAt)}</span>
            </div>
          </div>
        ))}
      </div>
