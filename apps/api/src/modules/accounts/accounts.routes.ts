...
import { CreateAccountSchema, UpdateAccountSchema } from './accounts.types.js'
...
export async function accountsRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: unknown }>('/', { preHandler: [authenticate] }, async (req, reply) => {
    try {
      ...
      // MANUAL n'a pas d'adapter (voir broker-sync.worker.ts) : le mettre en
      // queue échouerait systématiquement et polluerait le compte d'une
      // fausse "erreur de synchronisation" alors qu'il n'y a rien à synchroniser.
      if (body.brokerType !== 'MANUAL') {
  ...
  app.patch<{ Params: { id: string }; Body: unknown }>('/:id', { preHandler: [authenticate] }, async (req, reply) => {
    try {
      ...
      const account = await accountsService.update(req.params.id, req.user.id, body)
      return account
    } catch (err) {
      if (err instanceof ZodError) {
        return reply.code(400).send({ error: 'validation_error', details: err.errors })
  ...
  app.post<{ Params: { id: string } }>('/:id/sync', { preHandler: [authenticate] }, async (req, reply) => {
    const account = await accountsService.findById(req.params.id, req.user.id)
    if (!account) return reply.code(404).send({ error: 'account_not_found' })
    if (account.brokerType === 'MANUAL') {
      return reply.code(400).send({ error: 'manual_account_no_sync', detail: 'Ce compte est alimenté par import CSV, pas de synchro automatique.' })
    }
